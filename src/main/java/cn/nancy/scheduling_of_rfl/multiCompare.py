...
if __name__ == '__main__':
    ...
    for caseindex in range(casenum):
        # 前沿面比较
        path = ("E:\\JavaProjects\\zhengnanxi\\scheduling_of_rfl\\result\\00_case%d" % caselist[caseindex])
        ...
        for i in range(len(algorithemlist)):
            ...
            axs[2*caseindex][1].plot(range(len(bottomtotaldelay)), bottomtotaldelay, color=colorlist[i], linestyle='-.', label=legendlist[i])
            ...
            axs[2*caseindex+1][1].plot(range(len(toputilization)), toputilization, color=colorlist[i], linestyle='-.', label=legendlist[i])
    ...
    plt.savefig("E:\\JavaProjects\\zhengnanxi\\scheduling_of_rfl\\result\\Compare.jpg", format='jpg', dpi=300)
    plt.show()