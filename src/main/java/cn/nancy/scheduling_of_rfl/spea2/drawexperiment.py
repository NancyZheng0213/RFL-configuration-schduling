...
def readtable(design, result, objectiveList):
    xlsx = pd.ExcelFile('E:\\JavaProjects\\scheduling_of_rfl\\src\\main\\java\\cn\\nancy\\scheduling_of_rfl\\spea2\\experiment.xlsx')
...
if __name__ == '__main__':
    ...
    levelmaplenlist = [5, 5, 5, 3]
    fig, axs = plt.subplots(len(objectiveList), len(levelmap), sharey='row', sharex='col', figsize=(6,6), constrained_layout=True)
    colorlist = [[['darkgray' for _ in range(levelmaplenlist[j])] for j in range(len(levelmap))] for _ in range(len(objectiveList))]
    for i in range(len(colorlist)):
        colorlist[i][0][3] = 'C1'
        colorlist[i][1][4] = 'C1'
        colorlist[i][2][4] = 'C1'
        colorlist[i][3][2] = 'C1'
    bplotlist = [[] for _ in range(len(objectiveList))]
    for i in range(len(objectiveList)):
        ...
        for j in range(len(levelmap)):
            ...
            bplotlist[i].append(axs[i,j].boxplot(
                data[para],
                labels=labellist,
                patch_artist = True,
            ...
            for patch, color in zip(bplotlist[i][j]['boxes'], colorlist[i][j]):
                patch.set_facecolor(color)
        axs[i,0].set_ylabel(ylabellist[i],fontsize=14)
        axs[i,0].ticklabel_format(style='sci',scilimits=(0,2), axis='y', useMathText=True)

    for j in range(len(levelmap)):
        axs[i,j].set_xlabel(xlabellist[j], fontsize=14)
