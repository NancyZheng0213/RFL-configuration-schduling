    ...
    for machineindex in range(len(ConfigurationCode)):  # 机器序号
        ...
        for OP in S_table[machineindex]:                # 该机器上的操作
            for partindex in range(len(SortCode)):      # 工件序号
                ...
                task.append(SortCode[partindex])
                product.append('P' + str(SortCode[partindex]))
...
if __name__ == '__main__':
    ...
    # # %% 画甘特图
    # with open("E:\\JavaProjects\\configuration and scheduling of MRFL\\result\\case4_group1\\SPEA2_1\\2.txt", "r") as f:
    #     index = 1
    #     for line in f:
    ...
    for i in range(2,3):
        ...
        with open(p + "\\uOfM.txt") as f:
            for line in f.readlines():
                utilization.append(json.loads(line))
        with open(p + "\\tOfM.txt") as f:
            for line in f.readlines():
